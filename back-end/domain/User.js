...
class User {
    ...
    saveUser = async () => {
        ...
        const savedUser = await databaseUserAccess.saveUserInDB(this.email, this.password)
        if(savedUser.error) return new ApiErrors(savedUser.error).badRequest()
        return new Success().userCreated()        
    }
    findUser = async () => {
        const userInDB = await databaseUserAccess.findUserInDB((this.email))
    ...
    findAllOfThisUser = async (userId) => {
        ...
        return new Success().postFound({user:userInDB, posts: postsOfUser})
    }
    authorize = (token) => {
